...
let arr = str.filter((i) => {
return  i % 2 == 0
})  выбрать элементы которые поподают под наш кртерий (тру - попадает в новый массив/ фолс - не попадает в новый массив)

let newArray = array.every((item) => {
  return item <= 0
}) тру / фолс (ели хотябы один item НЕсоответсвует ритериям - фолс) [] - тру

let newArray = array.some((item) => {
  return item <= 0
}) тру / фолс (ели хотябы один item соответсвует ритериям - тру) [] - фолс

let newArray = array.reduce((total, item) => {
  return total + item
}, 0) total - накапливает значение. item - обычный элемент из массива. 0 - начальное значение тотала

let newArray = array.reduceRight((total, item) => {
  return total + item
}, 0) то же самое что и reduce только считает справа налево
...
Object.entries(obj) возвращает [ ['a', 1], ['b', 2] ]
Object.keys(obj) возвращает массив  объекта
Object.values(obj) возвращает массив значения
